...
begin
  ...
  -- Never silently claim an organisation already known to Selen. Existing records may
  -- belong to another service or historical dossier and must be linked by Selen staff.
  ...
  select jsonb_build_object(
    ...
    'users', case when can_users then coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'first_name', scp.first_name,
        'last_name', scp.last_name,
        'status', om.status,
        'primary_role', om.primary_role,
        'roles', coalesce((
          select jsonb_agg(r.role order by r.role)
          from public.organisation_membership_roles r
          where r.membership_id = om.id
        ), '[]'::jsonb),
        'permission_blocks', coalesce((
          select jsonb_agg(b.permission_block order by b.permission_block)
          from public.organisation_membership_permission_blocks b
          where b.membership_id = om.id and b.enabled = true and b.revoked_at is null
        ), '[]'::jsonb)
      ...
      select jsonb_agg(
        (to_jsonb(t) || jsonb_build_object(
          'certifications', coalesce((
            select jsonb_agg(to_jsonb(c) order by c.valid_until nulls last, c.created_at)
            from public.daily_trainer_certifications c
            where c.trainer_profile_id = t.id
          ), '[]'::jsonb)
...
revoke execute on function public.daily_client_workspace(uuid)
  from public, anon;
grant execute on function public.daily_client_workspace(uuid)
  to authenticated;
...
begin
  ...
  if cardinality(coalesce(p_roles, '{}'::text[])) = 0 then
  ...
  if not public.daily_can_assign_invitation_scope(
    ...
    coalesce(p_permission_blocks, '{}'::text[])
  ) then
    raise exception 'membership scope is not allowed for current user';
  end if;
  ...
  if target_membership.user_id = current_user_id then
    raise exception 'users cannot change their own access scope';
  end if;
  if target_membership.status not in ('active','disabled') then
    raise exception 'membership status cannot be changed here';
  end if;
  if exists (
    select 1 from public.organisation_membership_roles r
    where r.membership_id = target_membership.id and r.role = 'manager'
  ) then
  ...
  set enabled = false,
      revoked_at = now(),
      reason = 'Client access scope updated'
  where membership_id = target_membership.id
    and enabled = true
...
revoke execute on function public.daily_client_set_membership_access(uuid,uuid,text[],text[])
  from public, anon;
grant execute on function public.daily_client_set_membership_access(uuid,uuid,text[],text[])
  to authenticated;
...
begin
  ...
  if not public.can_invite_organisation_user(p_organisation_id) then
...
revoke execute on function public.daily_client_set_membership_status(uuid,uuid,text)
  from public, anon;
grant execute on function public.daily_client_set_membership_status(uuid,uuid,text)
  to authenticated;

-- A trainer may maintain their own unvalidated profile from the client workspace.
...
with check (
  ...
  and active = true
  and status not in ('validated','rejected')
  and selen_validated_at is null
  and selen_validated_by is null
);

-- Client managers and trainers may delete certification metadata they are already
-- allowed to manage. The later document-management lot will govern certificate files.
