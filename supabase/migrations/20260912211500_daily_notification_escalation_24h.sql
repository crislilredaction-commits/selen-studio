-- Selen Daily — align Studio checklist notification escalation with the validated 24-hour rule.
-- This migration only changes the escalation delay. It does not alter Auth, RLS,
-- assignments, notification targeting, link paths, or checklist business states.
...
begin
  ...
  -- A checklist item in todo is not yet an agent action. It only becomes
  -- actionable in Studio once there is something to review or a blocker.
...
begin
  ...
  values('daily_session_checklist',case item.status when 'blocked' then 'Dossier de session bloqué' when 'to_review' then 'Dossier de session à vérifier' else 'Action dossier de session' end,coalesce(formation_title,'Session Daily')||' · '||item.label,org_name,'/agent/daily/session-dossiers/'||item.session_id::text,case when assignment_agent_id is null then 'admin' else 'agent' end,assignment_user_id,assignment_agent_id,false,null,null,item.signaled_at+interval '24 hours',item.signaled_at,source_key_value,'daily_session_checklist')
  on conflict(source_key) where source_key is not null do update set title=excluded.title,content=excluded.content,organisation_name=excluded.organisation_name,link_path=excluded.link_path,target_role=excluded.target_role,target_user_id=excluded.target_user_id,target_agent_profile_id=excluded.target_agent_profile_id,escalation_at=excluded.escalation_at,dismissed_at=null,read_at=case when public.notifications.dismissed_at is not null or public.notifications.target_agent_profile_id is distinct from excluded.target_agent_profile_id or public.notifications.title is distinct from excluded.title or public.notifications.content is distinct from excluded.content then null else public.notifications.read_at end;
end;
$function$;

-- Recalculate open checklist notifications so existing rows adopt the new delay.
do $block$
declare
  checklist_id uuid;
begin
  for checklist_id in
    select id from public.daily_organisation_checklist_items where status in ('to_review','blocked')
  loop
    perform public.daily_sync_checklist_notification(checklist_id);
  end loop;

  for checklist_id in
    select id from public.daily_session_checklist_items where status in ('todo','to_review','blocked')
  loop
