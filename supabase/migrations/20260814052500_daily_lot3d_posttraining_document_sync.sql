...
-- Additive only: no existing document, attendance record, enrolment or storage object is deleted.
...
begin
  ...
  if linked_org is null then
    raise exception 'Daily post-training linked object not found';
  end if;
  if linked_org <> new.organisation_id then
    raise exception 'Daily post-training document organisation mismatch';
  end if;
  return new;
end;
$$;
...
as $$
  select case
    when p_document_type = 'attendance_summary' and p_linked_object_type = 'session'
...
begin
  ...
  select count(*) into active_enrolments
  from public.daily_session_enrolments e
  ...
  expected_documents := case
    when active_slots > 0 then 1 + eligible_certificates
    else 0
  end;
  ...
  with required_docs as (
    ...
    where d.is_current = true
      and d.document_type = 'attendance_summary'
  ...
  into current_documents, validated_documents
  from required_docs;
...
using (
  document_type in ('attendance_summary','completion_certificate')
