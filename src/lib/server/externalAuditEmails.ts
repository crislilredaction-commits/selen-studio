...
function auditHours(audit: Pick<ExternalAuditRow, "start_time" | "end_time">) {
  return `${audit.start_time.slice(0, 5)}${audit.end_time ? ` - ${audit.end_time.slice(0, 5)}` : ""}`;
}
...
function auditEndHour(audit: Pick<ExternalAuditRow, "audit_date" | "start_time" | "end_time">) {
...
function isCertifopac(certifier?: string | null) {
  return String(certifier ?? "").trim().toLowerCase().includes("certifopac");
}
...
function auditContactBlock() {
  const phone = process.env.LIL_AUDIT_PHONE?.trim() || process.env.LIL_CONTACT_PHONE?.trim() || "";
  return [
    "Coordonnées utiles :",
    "Pascale Barthaux",
...
function auditSummaryBlock(audit: ExternalAuditRow, remote: boolean, meetLink: string) {
  const certifier = audit.certifier || "à confirmer";
  return [
...
function formatSentStatus(value?: string | null) {
  ...
  return `Envoyé le ${new Intl.DateTimeFormat("fr-FR", { dateStyle: "medium", timeStyle: "short" }).format(new Date(value))}`;
}
...
export function buildExternalAuditConfirmationEmail(audit: ExternalAuditRow) {
  ...
  const certifopacParagraph = isCertifopac(audit.certifier)
    ? [
        "Par ailleurs, si cela n'est pas déjà fait, je vous invite à compléter le questionnaire disponible sur l'application Certifopac.",
        "Si vous rencontrez la moindre difficulté pour y accéder ou le compléter, n'hésitez pas à me contacter ; je vous accompagnerai avec plaisir.",
      ]
    : [];
  ...
  const bodyLines = remote
    ? [
        ...
        "Je vous contacte car la réalisation de votre audit Qualiopi m'a été confiée.",
        "Je me présente : je suis Pascale Barthaux, auditrice Qualiopi, et j'aurai le plaisir de vous accompagner lors de cet audit à distance.",
        "Voici un récapitulatif des informations prévues :",
        summary,
        "Je vous invite à vous connecter quelques minutes avant le début afin que nous puissions démarrer à l'heure prévue.",
        contacts,
        ...certifopacParagraph,
        "Si vous avez des questions concernant l'organisation pratique de l'audit, n'hésitez pas à me répondre directement.",
        "Pour toute question relative aux aspects contractuels ou financiers, je vous invite en revanche à contacter directement votre certificateur.",
        "Je vous remercie par avance et vous souhaite une excellente préparation.",
        "Bien cordialement,",
        "Pascale Barthaux\nAuditrice Qualiopi",
      ]
    : [
        ...
        "J'espère que vous allez bien.",
        "Je vous contacte car la réalisation de votre audit Qualiopi m'a été confiée.",
        "Je me présente : je suis Pascale Barthaux, auditrice Qualiopi, et j'aurai le plaisir de vous accompagner lors de cet audit.",
        "Voici un récapitulatif des informations prévues :",
        summary,
        contacts,
        "J'arriverai entre 5 et 15 minutes avant le début de l'audit afin que nous puissions nous installer sereinement.",
        "Pour le bon déroulement de la journée, il vous suffit de prévoir :\n\n- une table et des chaises ;\n- une prise de courant à proximité ;\n- une connexion Wi-Fi si possible.",
        "Il n'est pas nécessaire d'imprimer les documents à présenter : les supports numériques sont tout à fait acceptés.",
        "Afin de préparer le plan d'audit, pourriez-vous également m'indiquer, par retour de mail, les personnes qui seront présentes lors de l'audit ainsi que leur fonction au sein de l'organisme de formation ?",
        ...certifopacParagraph,
        "Si vous avez des questions concernant l'organisation pratique de l'audit, n'hésitez pas à me répondre directement.",
        "Pour toute question relative aux aspects contractuels ou financiers, je vous invite en revanche à contacter directement votre certificateur.",
        "Je vous remercie par avance pour votre accueil et vous souhaite une excellente préparation d'ici notre rencontre.",
...
export async function sendExternalAuditConfirmation(audit: ExternalAuditRow, options: { subject?: string | null; bodyText?: string | null } = {}) {
  const email = renderExternalAuditConfirmationEmail({ audit, subject: options.subject, bodyText: options.bodyText });
  if (!email.to) return { sent: false, error: "Email contact absent." };
  return sendClientEmailWithSilence({ email: email.to, to: email.to, subject: email.subject, html: email.html, text: email.text });
}

export function buildExternalAuditClientReminderEmail(audit: ExternalAuditRow) {
  const contact = audit.contact_name?.trim() || "Madame, Monsieur";
  const date = auditDateLabel(audit);
  const remote = isRemoteAudit(audit);
  const meetLink = getAuditMeetLink(audit);
  const subject = `Rappel audit Qualiopi - ${audit.of_name}`;
  const bodyLines = remote
    ? [`Bonjour ${contact},`, "Je vous rappelle que votre audit Qualiopi à distance est prévu demain.", `Date : ${date}`, `Heure de début : ${auditStartHour(audit)}`, `Heure de fin : ${auditEndHour(audit)}`, `Lien Google Meet : ${meetLink || "lien en cours de génération"}`, "Je vous invite à vous connecter quelques minutes avant le début afin que nous puissions démarrer sereinement.", auditContactBlock(), "Bien cordialement,", "Pascale Barthaux\nAuditrice Qualiopi"]
    : [`Bonjour ${contact},`, "Je vous rappelle que votre audit Qualiopi en présentiel est prévu demain.", `Date : ${date}`, `Heure de début : ${auditStartHour(audit)}`, `Heure de fin : ${auditEndHour(audit)}`, `Adresse : ${audit.address || "adresse à confirmer"}`, auditContactBlock(), "L'audit commencera à l'heure prévue.", "En cas de retard exceptionnel lié à la circulation, je vous préviendrai par SMS.", "Bien cordialement,", "Pascale Barthaux\nAuditrice Qualiopi"];
  const bodyText = bodyLines.join("\n\n");
  const rendered = renderSelenEmailFromText({ title: subject, bodyText, ctaLabel: remote && meetLink ? "Rejoindre Google Meet" : undefined, ctaUrl: remote && meetLink ? meetLink : undefined });
  return { to: audit.contact_email || "", subject, bodyText, html: rendered.html, text: rendered.text };
}
...
export async function sendExternalAuditClientReminder(audit: ExternalAuditRow) {
  ...
  if (!email.to) return { sent: false, error: "Email contact absent.", to: email.to };
  const result = await sendClientEmailWithSilence({ email: email.to, to: email.to, subject: email.subject, html: email.html, text: email.text });
...
export async function sendExternalAuditLilReminder(audit: ExternalAuditRow) {
  ...
  const result = await sendSelenEmail({ to: email.to, subject: email.subject, html: email.html, text: email.text });
  return { ...result, to: email.to };
}
