...
export default async function SessionDossiersPage() {
  ...
  const { data: dossiers } = await admin.from("daily_session_dossiers").select("session_id,organisation_id,status,assigned_agent_profile_id,updated_at").order("updated_at", { ascending: false });
  ...
  const [{ data: sessions }, { data: organisations }, { data: items }, { data: agents }] = await Promise.all([
    ...
    sessionIds.length ? admin.from("daily_session_checklist_items").select("session_id,status").in("session_id", sessionIds) : Promise.resolve({ data: [] }),
  ...
  return <main style={{ maxWidth: 1180, margin: "0 auto", padding: 28 }}>
    ...
    <p style={{ color: "var(--selen-text2)", marginTop: 0 }}>Checklist opérationnelle avant, pendant et après chaque formation.</p>
    <div style={{ display: "grid", gap: 12 }}>
      {(dossiers ?? []).map((d) => {
        const session = sessionMap.get(d.session_id); const org = orgMap.get(d.organisation_id); const formation = session ? formationMap.get(session.formation_id) : null; const agent = d.assigned_agent_profile_id ? agentMap.get(d.assigned_agent_profile_id) : null;
        const checklist = (items ?? []).filter((i) => i.session_id === d.session_id); const open = checklist.filter((i) => !["validated","not_applicable"].includes(i.status)).length;
        return <SelenCard key={d.session_id}><SelenCardTitle>{formation?.title ?? "Session Daily"}</SelenCardTitle>
          <div style={{ display: "flex", gap: 16, flexWrap: "wrap", fontSize: 13 }}><span>{org?.name ?? "OF"}</span><span>{session?.internal_reference || "Sans référence"}</span><span>{open} point(s) ouvert(s)</span><span>Agent : {agent ? `${agent.first_name ?? ""} ${agent.last_name ?? ""}`.trim() || agent.email : "Non assigné"}</span></div>
          <div style={{ marginTop: 12 }}><Link href={`/agent/daily/session-dossiers/${d.session_id}`}>Ouvrir le dossier →</Link></div>
        </SelenCard>;
      })}
