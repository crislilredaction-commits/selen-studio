...
async function reviewValidation(formData: FormData) {
  ...
  const { data: request, error: requestError } = await admin
    ...
    .select("id,organisation_id,status,proposed_changes")
    .eq("id", requestId)
  ...
  const { error } = await admin.from("daily_organisation_profile_change_requests").update({
    status: decision,
    reviewed_by: actor,
...
export default async function DailyOrganisationPage({ params, searchParams }: PageProps) {
  ...
  const [organisationRes, assignmentRes, agentsRes, checklistRes, membershipsRes, rolesRes, blocksRes, invitationsRes, trainersRes, certificationsRes, validationsRes, historyRes] = await Promise.all([
  ...
  return (
    <main style={s.page}>
        ...
        <form action={assignAgent} style={s.assignmentForm}>
          ...
          <select name="agent_profile_id" defaultValue={assignment?.agent_profile_id || ""} style={s.select} required>
            ...
            {(agentsRes.data ?? []).map((agent) => (
              <option value={agent.id} key={agent.id}>{[agent.first_name, agent.last_name].filter(Boolean).join(" ") || agent.email}</option>
            ))}
          </select>
      ...
      <section style={s.stats}>
        ...
        <MiniStat label="Agent" value={assignedAgent ? ([assignedAgent.first_name, assignedAgent.last_name].filter(Boolean).join(" ") || assignedAgent.email) : "Non assigné"} />
      </section>

      <nav style={s.tabs}>
        {tabs.map((item) => <Link key={item.key} href={`/agent/daily/organisations/${id}?tab=${item.key}`} style={{ ...s.tab, ...(tab === item.key ? s.activeTab : {}) }}>{item.label}</Link>)}
      </nav>

      {tab === "overview" ? (
        <div style={s.twoCols}>
          <SelenCard>
            <SelenCardTitle>À traiter maintenant</SelenCardTitle>
            {attention.length === 0 ? <p style={s.muted}>Rien de bloquant pour le moment. Profitons de ce phénomène rare.</p> : attention.map((item) => (
              <div key={item.id} style={s.attentionRow}>
                <div><strong>{item.label}</strong><p style={s.mutedInline}>{item.description}</p></div>
                <SelenBadge variant={item.status === "blocked" ? "danger" : "warn"}>{item.status === "blocked" ? "Bloqué" : item.status === "to_review" ? "À vérifier" : `À faire · ${daysSince(item.signaled_at)} j`}</SelenBadge>
              </div>
            ))}
          </SelenCard>
          <SelenCard>
            <SelenCardTitle>Signaux dossier</SelenCardTitle>
            <Signal label="NDA" value={organisation.nda_status || "unknown"} />
            <Signal label="Qualiopi" value={organisation.qualiopi_status || "unknown"} />
            <Signal label="Utilisateurs actifs" value={String(memberships.filter((m) => m.status === "active").length)} />
            <Signal label="Formateurs actifs" value={String(trainers.filter((t) => t.active).length)} />
            <Signal label="Invitations en attente" value={String(invitations.filter((i) => i.status === "pending").length)} />
          </SelenCard>
          <SelenCard style={{ gridColumn: "1 / -1" }}>
            <SelenCardTitle>Échéances formateurs</SelenCardTitle>
            {expiring.length === 0 ? <p style={s.muted}>Aucune certification à durée limitée n’arrive à échéance dans les 90 jours.</p> : expiring.map((cert) => {
              const trainer = trainers.find((t) => t.id === cert.trainer_profile_id);
              return <div key={cert.id} style={s.attentionRow}><span>{trainer?.display_name || "Formateur"} · {cert.title}</span><SelenBadge variant={certVariant(cert.validity_mode, cert.valid_until)}>{certificationLabel(cert.validity_mode, cert.valid_until)}</SelenBadge></div>;
            })}
          </SelenCard>
        </div>
      ) : null}
      ...
      {tab === "checklist" ? (
        <SelenCard>
          <SelenCardTitle>Checklist du dossier</SelenCardTitle>
          <div style={s.list}>{checklist.map((item) => (
            <form action={updateChecklist} key={item.id} style={s.checkRow}>
              <input type="hidden" name="organisation_id" value={id} />
              <input type="hidden" name="item_id" value={item.id} />
              <div style={{ flex: 1, minWidth: 220 }}><strong>{item.label}</strong><p style={s.mutedInline}>{item.description}</p><p style={s.tiny}>Signalé il y a {daysSince(item.signaled_at)} jour(s)</p></div>
              <select name="status" defaultValue={item.status} style={s.select}>
                <option value="todo">À faire</option><option value="in_progress">En cours</option><option value="to_review">À vérifier</option><option value="validated">Validé</option><option value="blocked">Bloqué</option><option value="not_applicable">Non applicable</option>
              </select>
              <input name="note" defaultValue={item.note || ""} placeholder="Note interne" style={s.input} />
              <SelenButton type="submit" size="sm">Enregistrer</SelenButton>
            </form>
          ))}</div>
        </SelenCard>
      ) : null}
      ...
      {tab === "info" ? (
        <div style={s.twoCols}>
          <InfoCard title="Identité" rows={[["Nom affiché", organisation.name],["Raison sociale", organisation.legal_name],["Forme juridique", organisation.legal_form],["SIRET", organisation.siret],["TVA", organisation.vat_number]]} />
          <InfoCard title="Coordonnées administratives" rows={[["Email", organisation.administrative_email || organisation.email],["Téléphone", organisation.administrative_phone || organisation.phone],["Adresse", organisation.administrative_address || organisation.address],["Contact", organisation.contact_name]]} />
          <InfoCard title="Représentant légal" rows={[["Nom", organisation.legal_representative_name],["Email", organisation.legal_representative_email]]} />
          <InfoCard title="Conformité" rows={[["NDA", organisation.nda_number],["Statut NDA", organisation.nda_status],["Déclaré le", organisation.nda_declared_at],["Qualiopi", organisation.qualiopi_status],["Valide du", organisation.qualiopi_valid_from],["Valide jusqu’au", organisation.qualiopi_valid_until],["Catégories", Array.isArray(organisation.qualiopi_categories) ? organisation.qualiopi_categories.join(", ") : null]]} />
        </div>
      ) : null}
      ...
      {tab === "users" ? (
        <div style={s.stack}>
          <SelenCard><SelenCardTitle>Utilisateurs de l’organisme</SelenCardTitle>{memberships.length === 0 ? <p style={s.muted}>Aucun utilisateur rattaché.</p> : memberships.map((membership) => {
            ...
            return <form action={updateMembership} key={membership.id} style={s.userRow}><input type="hidden" name="organisation_id" value={id} /><input type="hidden" name="membership_id" value={membership.id} /><div style={{ flex: 1 }}><strong>{membership.user_id}</strong><p style={s.mutedInline}>Rôles : {memberRoles.join(", ") || membership.primary_role || "—"}</p><p style={s.mutedInline}>Permissions : {memberBlocks.join(", ") || "aucun bloc"}</p></div><SelenBadge variant={membership.status === "active" ? "success" : "neutral"}>{membership.status}</SelenBadge><select name="status" defaultValue={membership.status} style={s.select}><option value="active">Actif</option><option value="disabled">Désactivé</option><option value="revoked">Révoqué</option></select><SelenButton type="submit" size="sm">Mettre à jour</SelenButton></form>;
          })}</SelenCard>
          <SelenCard><SelenCardTitle>Invitations</SelenCardTitle>{invitations.length === 0 ? <p style={s.muted}>Aucune invitation enregistrée.</p> : invitations.map((invite) => <div key={invite.id} style={s.attentionRow}><div><strong>{invite.invited_email}</strong><p style={s.mutedInline}>{(invite.intended_roles || []).join(", ")} · {(invite.intended_permission_blocks || []).join(", ") || "sans bloc"}</p></div><SelenBadge variant={invite.status === "pending" ? "warn" : invite.status === "accepted" ? "success" : "neutral"}>{invite.status}</SelenBadge></div>)}<p style={s.tiny}>L’envoi et le parcours public d’acceptation seront raccordés au côté client Daily ; le Studio affiche déjà l’état et le périmètre sécurisé des invitations.</p></SelenCard>
        </div>
      ) : null}

      {tab === "trainers" ? (
        <div style={s.stack}>{trainers.length === 0 ? <SelenCard><p style={s.muted}>Aucun formateur enregistré.</p></SelenCard> : trainers.map((trainer) => {
          const trainerCerts = certifications.filter((cert) => cert.trainer_profile_id === trainer.id);
          return <SelenCard key={trainer.id}><div style={s.trainerHead}><div><SelenCardTitle>{trainer.display_name || trainer.professional_email || "Formateur"}</SelenCardTitle><p style={s.mutedInline}>{trainer.engagement_type} · {trainer.professional_email || "email non renseigné"}</p></div><SelenBadge variant={trainer.status === "validated" ? "success" : trainer.status === "pending_selen_review" ? "warn" : "neutral"}>{trainer.status}</SelenBadge></div><div style={s.certList}>{trainerCerts.map((cert) => <div key={cert.id} style={s.certRow}><div><strong>{cert.title}</strong><p style={s.mutedInline}>{[cert.issuer, cert.reference, cert.obtained_on ? `obtenue le ${cert.obtained_on}` : null].filter(Boolean).join(" · ")}</p></div><SelenBadge variant={certVariant(cert.validity_mode, cert.valid_until)}>{certificationLabel(cert.validity_mode, cert.valid_until)}</SelenBadge><form action={deleteCertification}><input type="hidden" name="organisation_id" value={id} /><input type="hidden" name="certification_id" value={cert.id} /><button type="submit" style={s.deleteButton}>Supprimer</button></form></div>)}</div><form action={addCertification} style={s.certForm}><input type="hidden" name="organisation_id" value={id} /><input type="hidden" name="trainer_profile_id" value={trainer.id} /><input name="title" required placeholder="Certification / habilitation" style={s.input} /><input name="issuer" placeholder="Organisme certificateur" style={s.input} /><input name="reference" placeholder="Référence (facultatif)" style={s.input} /><label style={s.fieldLabel}>Date d’obtention<input type="date" name="obtained_on" style={s.input} /></label><label style={s.fieldLabel}>Validité<select name="validity_mode" defaultValue="unknown" style={s.select}><option value="unknown">Inconnue</option><option value="lifetime">À vie</option><option value="limited">Durée limitée</option></select></label><label style={s.fieldLabel}>Fin de validité<input type="date" name="valid_until" style={s.input} /></label><SelenButton type="submit" size="sm">Ajouter la certification</SelenButton></form></SelenCard>;
        })}</div>
      ) : null}

      {tab === "validations" ? (
        <SelenCard><SelenCardTitle>Demandes de validation Selen</SelenCardTitle>{validations.length === 0 ? <p style={s.muted}>Aucune demande.</p> : validations.map((request) => <article key={request.id} style={s.validationRow}><div style={{ flex: 1 }}><div style={s.badgeLine}><SelenBadge variant={request.status === "pending" ? "warn" : request.status === "approved" ? "success" : "neutral"}>{request.status}</SelenBadge><strong>{request.request_type}</strong></div><pre style={s.pre}>{JSON.stringify(request.proposed_changes, null, 2)}</pre>{request.review_message ? <p style={s.mutedInline}>Décision : {request.review_message}</p> : null}</div>{request.status === "pending" ? <form action={reviewValidation} style={s.reviewForm}><input type="hidden" name="organisation_id" value={id} /><input type="hidden" name="request_id" value={request.id} /><textarea name="message" placeholder="Motif / commentaire" style={s.textarea} /><div style={s.badgeLine}><button type="submit" name="decision" value="approved" style={s.approveButton}>Valider</button><button type="submit" name="decision" value="rejected" style={s.rejectButton}>Refuser</button></div></form> : null}</article>)}</SelenCard>
      ) : null}

      {tab === "history" ? (
        <SelenCard><SelenCardTitle>Historique Daily</SelenCardTitle>{(historyRes.data ?? []).length === 0 ? <p style={s.muted}>Aucun événement d’audit enregistré.</p> : (historyRes.data ?? []).map((event) => <div key={event.id} style={s.historyRow}><div><strong>{event.action}</strong><p style={s.mutedInline}>{event.object_type} · {event.actor_role || event.actor_type}</p>{event.reason ? <p style={s.mutedInline}>{event.reason}</p> : null}</div><span style={s.tiny}>{new Date(event.occurred_at).toLocaleString("fr-FR")}</span></div>)}</SelenCard>
      ) : null}
    </main>
  );
}
...
const s: Record<string, CSSProperties> = {
  ...
  back: { color: "var(--selen-text3)", fontSize: 12, textDecoration: "none" }, eyebrow: { marginTop: 10, fontSize: 9, letterSpacing: ".28em", textTransform: "uppercase", color: "var(--selen-gold)" }, title: { fontFamily: "var(--font-display)", fontSize: 30, margin: "5px 0" }, subtitle: { fontSize: 13, color: "var(--selen-text2)" },
  assignmentForm: { display: "flex", gap: 8, alignItems: "end", flexWrap: "wrap" }, smallLabel: { fontSize: 10, color: "var(--selen-text3)" }, select: { background: "var(--selen-bg3)", color: "var(--selen-text)", border: "1px solid var(--selen-border)", borderRadius: 10, padding: "9px 10px", minHeight: 38 }, input: { background: "var(--selen-bg3)", color: "var(--selen-text)", border: "1px solid var(--selen-border)", borderRadius: 10, padding: "9px 10px", minWidth: 150 },
  stats: { display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(150px,1fr))", gap: 10, margin: "18px 0" }, statCard: { minHeight: 82, display: "flex", flexDirection: "column", justifyContent: "space-between" }, tabs: { display: "flex", gap: 7, flexWrap: "wrap", marginBottom: 16 }, tab: { color: "var(--selen-text2)", textDecoration: "none", fontSize: 12, border: "1px solid var(--selen-border)", borderRadius: 999, padding: "8px 12px" }, activeTab: { color: "var(--selen-ink)", background: "linear-gradient(135deg,var(--selen-gold),var(--selen-copper))", borderColor: "transparent", fontWeight: 700 },
  ...
  checkRow: { display: "flex", gap: 9, alignItems: "center", flexWrap: "wrap", padding: 12, border: "1px solid var(--selen-border)", borderRadius: 14, background: "var(--selen-bg3)" }, userRow: { display: "flex", gap: 9, alignItems: "center", flexWrap: "wrap", padding: "11px 0", borderBottom: "1px solid var(--selen-border)" }, trainerHead: { display: "flex", justifyContent: "space-between", gap: 12, alignItems: "flex-start" }, certList: { display: "grid", marginTop: 8 }, certRow: { display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, flexWrap: "wrap", borderTop: "1px solid var(--selen-border)", padding: "10px 0" }, certForm: { display: "flex", gap: 8, flexWrap: "wrap", alignItems: "end", marginTop: 14, paddingTop: 14, borderTop: "1px solid var(--selen-border)" }, fieldLabel: { display: "grid", gap: 5, fontSize: 10, color: "var(--selen-text3)" }, deleteButton: { border: 0, background: "transparent", color: "#f0a0a0", cursor: "pointer", fontSize: 11 },
