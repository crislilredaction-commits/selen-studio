...
export default async function DailyStudioLayout({ children }: { children: ReactNode }) {
  ...
  return <>
    ...
    <nav className="daily-subnav" aria-label="Navigation Selen Daily" style={{maxWidth:1180,margin:"14px auto 0",padding:"0 28px",display:"flex",gap:8,flexWrap:"wrap"}}>
      ...
      <Link href="/agent/daily/qualite" style={linkStyle}>Veille & Qualité</Link>
      {owner ? <Link href="/agent/daily/generateur-formation" style={linkStyle}>Générateur formation</Link> : null}
    </nav>
    <div className="daily-content">{children}</div>
  </>;
}

const linkStyle={textDecoration:"none",fontSize:11,fontWeight:600,color:"var(--selen-text2)",border:"1px solid var(--selen-border)",borderRadius:999,padding:"8px 12px",minHeight:40,display:"inline-flex",alignItems:"center",boxSizing:"border-box" as const,background:"var(--selen-bg2)",whiteSpace:"nowrap" as const};
const priorityLinkStyle={...linkStyle,color:"var(--selen-gold2)",border:"1px solid var(--selen-border2)"};
