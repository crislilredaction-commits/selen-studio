...
export async function GET(req:Request) {
  ...
  const { data:document,error } = await admin.from("daily_documents").select("bucket,storage_path,document_type").eq("id",id).in("document_type",["training_program","training_agreement","convocation","registration_positioning"]).single();
