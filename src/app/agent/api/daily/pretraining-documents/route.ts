...
export async function GET() {
  ...
  const { data, error } = await admin.from("daily_documents").select("*,organisations(name)").in("document_type",types).eq("is_current",true).order("created_at",{ascending:false});
...
export async function PATCH(req:Request) {
  ...
  const id = String(body.id ?? ""); const action = String(body.action ?? ""); const note = typeof body.note === "string" ? body.note.trim() : "";
  if (!id || !["validate","request_correction"].includes(action)) return NextResponse.json({error:"Action invalide."},{status:400});
  const supabase = await createClient(); const { data:userData } = await supabase.auth.getUser(); const userId = userData.user?.id;
  if (!userId) return NextResponse.json({error:"Utilisateur introuvable."},{status:401});
  const admin = createSupabaseAdminClient();
  const { data:current,error:readError } = await admin.from("daily_documents").select("id,status,metadata").eq("id",id).in("document_type",types).eq("is_current",true).single();
