...
function programmePositionnement(f: FormState) {
  ...
  for (const person of people(f.beneficiaires)) {
    ...
    doc.setFontSize(18);
    doc.setTextColor(130, 92, 35);
    doc.text("Inscription et positionnement", M, M);
...
function convention(f: FormState) {
  ...
  y = text(doc, "L’assiduité est tracée au moyen d’une feuille d’émargement. Les preuves de réalisation et d’évaluation sont conservées dans le dossier de la session.", y);
  ...
  y = text(doc, "Les conditions d’annulation, de report, de règlement et les clauses propres à l’organisme de formation sont à compléter selon ses conditions habituelles.", y);
...
function convocations(f: FormState) {
  ...
  people(f.beneficiaires).forEach((person, index) => {
    ...
    doc.setFontSize(18);
    doc.setTextColor(130, 92, 35);
    doc.text("Convocation à la formation", M, M);
    let y = 30;
    y = text(doc, `Madame, Monsieur ${person},`, y);
    y = text(doc, `Nous vous confirmons votre inscription à la formation « ${fallback(f.formationTitre)} ».`, y);
    y = section(doc, "Informations pratiques", y);
    y = field(doc, "Dates", period(f), y);
    y = field(doc, "Horaires", f.horaires, y);
    y = field(doc, "Durée", f.formationDuree, y);
    ...
    y = field(doc, "Modalité", f.formationModalite, y);
    y = field(doc, "Formateur", f.formateur, y);
    y = text(doc, "Merci de prévenir l’organisme de formation en cas d’empêchement ou de besoin particulier nécessitant une adaptation.", y);
    y = section(doc, "Contact", y);
    text(doc, `${fallback(f.organismeNom)}\n${fallback(f.organismeEmail)}\n${fallback(f.organismeTelephone)}`, y);
  });
  return doc;
}
...
function emargement(f: FormState) {
  ...
  allDates(f.dateDebut, f.dateFin).forEach((date, day) => {
    ...
    doc.setFontSize(18);
    doc.setTextColor(130, 92, 35);
    doc.text("Feuille d’émargement", M, M);
    let y = 30;
...
function satisfaction(f: FormState) {
  ...
  people(f.beneficiaires).forEach((person, index) => {
    ...
    doc.setFontSize(18);
    doc.setTextColor(130, 92, 35);
    doc.text("Questionnaire de satisfaction", M, M);
    let y = 30;
    y = field(doc, "Formation", f.formationTitre, y);
...
function suivi(f: FormState) {
  ...
  people(f.beneficiaires).forEach((person, index) => {
    ...
    doc.setFontSize(18);
    doc.setTextColor(130, 92, 35);
    doc.text("Fiche de suivi pédagogique", M, M);
    let y = 30;
    y = field(doc, "Formation", f.formationTitre, y);
    y = field(doc, "Bénéficiaire", person, y);
    y = field(doc, "Formateur", f.formateur, y);
    ...
    for (const h of ["Progression et observations", "Difficultés éventuellement rencontrées", "Adaptations pédagogiques mises en œuvre", "Acquis / compétences observés", "Actions ou recommandations complémentaires"]) {
      y = section(doc, h, y);
      y = blanks(doc, y, 4);
    }
...
function Card({ title, children }: { title: string; children: React.ReactNode }) {
  return <section className="rounded-3xl border border-stone-800 bg-stone-900/90 p-6"><h2 className="mb-5 text-lg font-semibold text-stone-100">{title}</h2>{children}</section>;
}
  ...
  const generate = () => {
    ...
    try {
      ...
      const docs: Array<[string, jsPDF]> = [
        ...
        ["02_Convention_Formation.pdf", convention(f)],
        ["03_Convocations.pdf", convocations(f)],
