...
function appendReglementInterieur(doc: jsPDF, f: FormState, annexLabel = "Annexe — Règlement intérieur") {
  ...
  y = text(
    doc,
    `Le présent règlement intérieur est établi pour les actions de formation organisées par ${fallback(f.organismeNom)}. Il s’applique à l’ensemble des stagiaires pendant toute la durée de leur formation. Il complète les règles propres au lieu d’accueil lorsqu’elles existent.`,
    y,
    false,
    9,
  );
  ...
  const blocks: Array<[string, string]> = [
    [
      ...
      "Le règlement précise les règles générales relatives à la santé et à la sécurité, à la discipline et, lorsque la durée de l’action le justifie, à la représentation des stagiaires. Chaque stagiaire s’engage à le respecter dès son entrée en formation.",
    ],
    [
      "2. Règles du lieu d’accueil",
      "Lorsque la formation se déroule dans une entreprise, un établissement ou des locaux disposant de leurs propres consignes ou règlement intérieur, les règles de santé et de sécurité de ce lieu s’appliquent en priorité sur ces matières. Les stagiaires doivent respecter les consignes affichées ou communiquées sur place.",
    ],
    [
      "3. Santé, sécurité et prévention",
      "Chaque stagiaire doit prendre soin de sa sécurité et de celle des autres, respecter les consignes de prévention, utiliser correctement les équipements mis à disposition et signaler sans délai au formateur ou à l’organisme toute situation présentant un risque, tout incident ou tout accident survenu pendant la formation ou à l’occasion de celle-ci.",
    ],
    [
      "4. Consignes d’urgence",
      "En cas d’alerte, d’incendie ou de situation nécessitant une évacuation, les stagiaires suivent immédiatement les instructions du responsable du lieu, du formateur ou des personnes chargées de la sécurité. Les accès, issues de secours et équipements de sécurité ne doivent jamais être encombrés ou détournés de leur usage.",
    ],
    [
      "5. Produits, alcool, tabac et comportement à risque",
      "Il est interdit de se présenter ou de demeurer en formation dans un état incompatible avec la sécurité ou le bon déroulement de la session. La consommation d’alcool ou de substances illicites pendant la formation est interdite. Les règles applicables au tabac et au vapotage sont celles du lieu d’accueil et de la réglementation en vigueur.",
    ],
    [
      "6. Utilisation des locaux, matériels et outils numériques",
...
function appendLivretAccueil(doc: jsPDF, f: FormState, person: string) {
  ...
  y = text(
    doc,
    `Bienvenue dans cette action de formation organisée par ${fallback(f.organismeNom)}. Ce livret rassemble les informations pratiques communiquées pour préparer votre accueil et le bon déroulement de la session.`,
    y,
    false,
    9,
  );
  ...
  y = text(
    doc,
    `Les méthodes et moyens pédagogiques prévus sont les suivants : ${fallback(f.formationMethodes)}. Le formateur peut adapter le rythme, les exemples ou les activités dans le respect des objectifs annoncés et des besoins identifiés.`,
    y,
    false,
    9,
  );
  ...
  y = text(
    doc,
    `Les acquis sont évalués selon les modalités suivantes : ${fallback(f.formationEvaluation)}. Le bénéficiaire participe aux activités et évaluations prévues afin de permettre le suivi de sa progression.`,
    y,
    false,
    9,
  );
  ...
  y = text(
    doc,
    `Le bénéficiaire se présente ou se connecte aux horaires indiqués, respecte les consignes transmises et signale dès que possible tout retard, absence ou difficulté. Pour une formation en présentiel, les consignes du lieu d’accueil s’appliquent. Pour une formation à distance, le bénéficiaire veille à disposer des moyens techniques nécessaires à sa participation et à rejoindre les espaces de formation communiqués par l’organisme.`,
    y,
    false,
    9,
  );
  ...
  y = text(
    doc,
    `${fallback(f.formationAccessibilite)}\n\nTout besoin d’adaptation non encore signalé peut être communiqué à l’organisme afin qu’une solution adaptée soit recherchée lorsque cela est possible.`,
    y,
    false,
    9,
  );
  ...
  y = text(
    doc,
    `Pour toute question avant ou pendant la formation, contactez ${fallback(f.organismeNom)} : ${fallback(f.organismeEmail)} · ${fallback(f.organismeTelephone)}. Toute difficulté concernant l’accueil, l’organisation ou le déroulement peut être signalée à ces mêmes coordonnées.`,
    y,
    false,
    9,
  );
  ...
  y = text(
    doc,
    "Le bénéficiaire respecte les consignes de santé et de sécurité du lieu d’accueil ainsi que le règlement intérieur applicable à la formation. Le règlement intérieur complet figure à la suite du présent livret.",
    y,
    false,
    9,
  );
  ...
  text(
    doc,
    "• Être présent ou connecté à l’heure prévue.\n• Prévoir le matériel personnel éventuellement nécessaire à la prise de notes.\n• Participer aux exercices, activités et évaluations prévues.\n• Signaler immédiatement toute difficulté pouvant empêcher le bon déroulement de la formation.",
    y,
    false,
    9,
  );
...
function convocations(f: FormState) {
  const doc = new jsPDF();
  const participants = people(f.beneficiaires);
  ...
  participants.forEach((person, index) => {
    ...
    y = text(
      doc,
      `Nous avons le plaisir de vous confirmer votre participation à la formation « ${fallback(f.formationTitre)} », organisée par ${fallback(f.organismeNom)}.`,
      y,
      false,
      10,
      5,
    ...
    y = text(
      doc,
      `La session se déroulera ${period(f)}, selon les horaires suivants : ${fallback(f.horaires)}. La durée annoncée est de ${fallback(f.formationDuree)}.`,
      y,
      false,
      10,
      5,
    ...
    y = text(
      doc,
      `Modalité : ${fallback(f.formationModalite)}. Lieu ou modalités d’accès : ${fallback(f.lieu)}. Votre formateur sera ${fallback(f.formateur)}.`,
      y,
      false,
      10,
      5,
    ...
    y = text(
      doc,
      "Nous vous remercions de vous présenter ou de vous connecter à l’heure prévue et de nous informer au plus tôt de tout empêchement, retard ou besoin particulier nécessitant une adaptation.",
      y,
      false,
      10,
      5,
    ...
    y = text(
      doc,
      `Pour toute question, vous pouvez contacter ${fallback(f.organismeNom)} à l’adresse ${fallback(f.organismeEmail)} ou au ${fallback(f.organismeTelephone)}.`,
      y,
      false,
      10,
      7,
...
function suivi(f: FormState) {
  ...
  people(f.beneficiaires).forEach((person, index) => {
    ...
    for (const h of [
      ...
      "Acquis / compétences observés",
      "Actions ou recommandations complémentaires",
    ]) {
      y = section(doc, h, y);
      y = blanks(doc, y, 4);
    }
...
function procedure(f: FormState) {
  ...
  const steps: Array<[string, string]> = [
    ...
    [
      "5. À la fin de la formation",
      "À faire compléter :\n• Questionnaire de satisfaction\n\nÀ récupérer et conserver :\n• Questionnaire de satisfaction\n• Feuille d’émargement complète\n• Fiche de suivi\n• Copie ou résultat de l’évaluation des acquis",
    ],
    [
      "6. Archivage",
      "Conserver ensemble : programme et positionnement, convention signée, règlement intérieur, convocation et livret d’accueil, émargement, fiche de suivi, évaluation des acquis ou preuve du résultat, questionnaire de satisfaction et tout échange utile permettant de retracer le déroulement de la formation.",
    ],
  ];
...
export default function DossierGeneratorClientV2() {
  ...
  const generate = () => {
    ...
    try {
      ...
      const docs: Array<[string, jsPDF]> = [
        ...
        ["04_Feuille_Emargement.pdf", emargement(f)],
  ...
  return <div className="px-8 py-10"><div className="mx-auto max-w-5xl"><header className="mb-8"><div className="flex items-center gap-3"><div className="rounded-2xl border border-amber-300/20 bg-amber-300/10 p-3 text-amber-300"><FileArchive size={22} /></div><div><p className="text-xs uppercase tracking-[0.25em] text-amber-300/80">Selen Studio</p><h1 className="mt-1 text-3xl font-semibold text-stone-100">Générateur de dossier de formation</h1></div></div><p className="mt-4 max-w-3xl text-sm leading-6 text-stone-400">Outil temporaire pour les premiers clients Selen. Les données ne sont pas enregistrées en base : elles servent uniquement à produire les documents préremplis à envoyer manuellement par email.</p></header>
    <div className="space-y-6">
      ...
      <section className="rounded-3xl border border-amber-300/20 bg-amber-300/[0.06] p-6"><h2 className="font-semibold text-stone-100">Documents générés</h2><p className="mt-2 text-sm leading-6 text-stone-400">Programme + inscription/positionnement, convention avec règlement intérieur complet en annexe, convocation sous forme de courrier avec livret d’accueil et règlement intérieur, feuille d’émargement, questionnaire de satisfaction, fiche de suivi pédagogique et procédure d’utilisation.</p><p className="mt-2 text-sm leading-6 text-stone-500">Chaque page comporte le pied de page de l’organisme avec ses coordonnées, son SIRET, son NDA, sa région d’enregistrement et la date d’actualisation. L’évaluation des acquis n’est pas générée : la procédure rappelle d’en conserver une copie ou une preuve du résultat.</p></section>
