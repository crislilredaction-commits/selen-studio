...
type FormState = {
  ...
  formationModalitesAcces: string;
  formationDelaiAcces: string;
  indicateurSatisfaction: string;
  indicateurApprenants: string;
  indicateurReussite: string;
  dateDebut: string;
  dateFin: string;
  horaires: string;
  lieu: string;
...
function emptyForm(): FormState {
  return {
    ...
    formationDuree: "",
    formationModalite: "",
    formationModalitesAcces: "Inscription après échange sur le besoin, positionnement et validation de l’inscription. La prise en charge financière est vérifiée le cas échéant.",
    formationDelaiAcces: "15 jours à compter de la validation de l’inscription, sous réserve de disponibilité et des éventuels délais du financeur.",
    indicateurSatisfaction: "",
    indicateurApprenants: "",
    indicateurReussite: "",
    dateDebut: "",
    dateFin: "",
    horaires: "",
...
function indicator(value: string, isRate = false) {
  const v = value.trim();
  if (!v) return "En cours";
  if (isRate && /^\d+(?:[,.]\d+)?$/.test(v)) return `${v} %`;
  return v;
}
...
function appendReglementInterieur(doc: jsPDF, f: FormState, annexLabel = "Annexe — Règlement intérieur") {
  ...
  const blocks: Array<[string, string]> = [
    ...
    ["2. Règles du lieu d’accueil", "Lorsque la formation se déroule dans une entreprise, un établissement ou des locaux disposant de leurs propres consignes ou règlement intérieur, les règles de santé et de sécurité de ce lieu s’appliquent en priorité. Les stagiaires respectent les consignes affichées ou communiquées sur place."],
    ["3. Santé, sécurité et prévention", "Chaque stagiaire prend soin de sa sécurité et de celle des autres, respecte les consignes de prévention, utilise correctement les équipements et signale sans délai toute situation présentant un risque, tout incident ou tout accident."],
    ["4. Consignes d’urgence", "En cas d’alerte, d’incendie ou de situation nécessitant une évacuation, les stagiaires suivent immédiatement les instructions du responsable du lieu, du formateur ou des personnes chargées de la sécurité."],
    ...
    ["7. Assiduité, horaires et absences", `Les stagiaires respectent les horaires communiqués pour la session, soit ${fallback(f.horaires)}. Toute absence, retard ou départ anticipé doit être signalé dans les meilleurs délais et peut devoir être justifié. L’assiduité est constatée au moyen des dispositifs prévus pour la modalité de formation.`],
    ["8. Comportement et respect des personnes", "Un comportement respectueux est attendu à l’égard des autres stagiaires, du formateur, des intervenants et de toute personne présente sur le lieu de formation. Les violences, menaces, insultes, discriminations, faits de harcèlement ou perturbations répétées sont incompatibles avec le déroulement de la formation."],
    ["9. Égalité de traitement, liberté de conscience et neutralité", "L’organisme veille à l’égalité de traitement des stagiaires et respecte leur liberté d’expression et de conscience dans les limites nécessaires au bon déroulement de la formation et au respect d’autrui. Lorsque les obligations légales liées au financement l’exigent, les enseignements sont dispensés dans le respect du principe de neutralité."],
    ["10. Confidentialité et propriété des supports", "Les informations confidentielles obtenues à l’occasion de la formation ne doivent pas être diffusées sans autorisation. Les supports pédagogiques restent soumis aux droits de leurs auteurs et ne peuvent être reproduits ou diffusés au-delà des usages autorisés."],
    ["11. Sanctions disciplinaires", "Tout comportement considéré comme fautif peut donner lieu, selon sa nature et sa gravité, à un rappel écrit des règles, un avertissement, une exclusion temporaire ou une exclusion définitive. Aucune sanction pécuniaire n’est appliquée et toute mesure doit être proportionnée aux faits reprochés."],
    ["12. Procédure disciplinaire", "Aucune sanction n’est prononcée sans que le stagiaire ait été préalablement informé des griefs retenus contre lui. Lorsqu’une sanction est susceptible d’avoir une incidence sur sa présence en formation, le stagiaire est convoqué à un entretien par écrit, peut se faire assister par la personne de son choix et présente ses explications. La décision est écrite, motivée et notifiée dans les délais applicables. Une mesure conservatoire peut être prise en cas de nécessité immédiate sans préjuger de la sanction définitive."],
    ["13. Représentation des stagiaires", "Pour les actions organisées en sessions d’une durée totale supérieure à 500 heures, un délégué titulaire et un délégué suppléant sont élus dans les conditions prévues par le Code du travail. Ils peuvent présenter les suggestions et réclamations relatives aux conditions de formation et à l’application du présent règlement."],
...
function appendLivretAccueil(doc: jsPDF, f: FormState, person: string) {
  ...
  y = text(doc, `Le bénéficiaire se présente ou se connecte aux horaires indiqués, respecte les consignes transmises et signale dès que possible tout retard, absence ou difficulté. Pour une formation en présentiel, les consignes du lieu d’accueil s’appliquent. Pour une formation à distance, le bénéficiaire veille à disposer des moyens techniques nécessaires à sa participation.`, y, false, 9);
...
function programmePositionnement(f: FormState) {
  ...
  y = field(doc, "Prérequis", f.formationPrerequis || "Aucun", y);
  y = field(doc, "Durée", f.formationDuree, y);
  y = field(doc, "Modalité pédagogique", f.formationModalite, y);
  y = field(doc, "Modalités d’accès / inscription", f.formationModalitesAcces, y);
  y = field(doc, "Délai d’accès", f.formationDelaiAcces, y);
  y = field(doc, "Dates", period(f), y);
  y = field(doc, "Horaires", f.horaires, y);
  y = field(doc, "Lieu", f.lieu, y);
  y = field(doc, "Formateur", f.formateur, y);
  y = field(doc, "Tarif", price(f.tarif), y);
  y = field(doc, "Contact", `${fallback(f.organismeEmail)} · ${fallback(f.organismeTelephone)}`, y);
  ...
  y = section(doc, "Accessibilité aux personnes en situation de handicap", y);
  y = text(doc, f.formationAccessibilite, y);
  y = section(doc, "Indicateurs de résultats", y);
  y = text(doc, `Taux de satisfaction : ${indicator(f.indicateurSatisfaction, true)}\nNombre d’apprenants : ${indicator(f.indicateurApprenants)}\nTaux de réussite : ${indicator(f.indicateurReussite, true)}\nDonnées actualisées au ${dateFr(f.dateActualisation)}.`, y, false, 9);
...
function convention(f: FormState) {
  ...
  y = text(doc, "Les conditions d’annulation, de report, de règlement et les clauses propres à l’organisme de formation sont à appliquer selon ses conditions contractuelles habituelles.", y);
...
function appendSupportReceipt(doc: jsPDF, f: FormState, persons: string[]) {
  ...
  const drawHeader = () => {
    ...
    headers.forEach((header, i) => {
      ...
      doc.text((doc.splitTextToSize(header, widths[i] - 4) as string[]).slice(0, 2), x + 2, y + 5);
      x += widths[i];
    });
    y += 11;
  };
  ...
  for (const person of persons) {
    ...
    widths.forEach((width, i) => {
      ...
      if (i === 0) doc.text((doc.splitTextToSize(person, width - 4) as string[]).slice(0, 3), x + 2, y + 7);
      if (i === 1) doc.text((doc.splitTextToSize("[ ] Je certifie avoir reçu en main propre les supports de formation remis dans le cadre de cette action.", width - 4) as string[]).slice(0, 5), x + 2, y + 6);
...
function Card({ title: cardTitle, description, children }: { title: string; description?: string; children: React.ReactNode }) {
  return <section className="rounded-3xl border border-stone-800 bg-stone-900/90 p-6"><h2 className="text-lg font-semibold text-stone-100">{cardTitle}</h2>{description ? <p className="mt-1 mb-5 text-sm text-stone-500">{description}</p> : <div className="mb-5" />}{children}</section>;
}

export default function DossierGeneratorClientV4() {
  ...
  return <div className="px-8 py-10"><div className="mx-auto max-w-5xl"><header className="mb-8"><div className="flex items-center gap-3"><div className="rounded-2xl border border-amber-300/20 bg-amber-300/10 p-3 text-amber-300"><FileArchive size={22} /></div><div><p className="text-xs uppercase tracking-[0.25em] text-amber-300/80">Selen Studio</p><h1 className="mt-1 text-3xl font-semibold text-stone-100">Générateur de dossier de formation</h1></div></div><p className="mt-4 max-w-3xl text-sm leading-6 text-stone-400">Outil temporaire pour les premiers clients Selen. Les données ne sont pas enregistrées en base : elles servent uniquement à produire les documents préremplis à envoyer manuellement par email.</p></header>
    <div className="space-y-6">
      ...
      <Card title="3. Formation" description="Les champs de cette section alimentent notamment les informations attendues par les indicateurs 1 et 2 du référentiel Qualiopi."><div className="space-y-4"><Field label="Intitulé de la formation *" value={f.formationTitre} onChange={(v) => set("formationTitre", v)} /><div className="grid gap-4 md:grid-cols-2"><Field label="Public visé" value={f.formationPublic} onChange={(v) => set("formationPublic", v)} /><Field label="Prérequis" value={f.formationPrerequis} onChange={(v) => set("formationPrerequis", v)} /><Field label="Durée" placeholder="Ex. 14 heures / 2 jours" value={f.formationDuree} onChange={(v) => set("formationDuree", v)} /><Field label="Modalité pédagogique" placeholder="Présentiel, distanciel..." value={f.formationModalite} onChange={(v) => set("formationModalite", v)} /></div><Area label="Modalités d’accès / inscription" rows={3} value={f.formationModalitesAcces} onChange={(v) => set("formationModalitesAcces", v)} /><Area label="Délai d’accès" rows={2} value={f.formationDelaiAcces} onChange={(v) => set("formationDelaiAcces", v)} /><Area label="Objectifs" value={f.formationObjectifs} onChange={(v) => set("formationObjectifs", v)} /><Area label="Programme complet" rows={16} placeholder="Colle ici le programme complet validé..." value={f.formationProgramme} onChange={(v) => set("formationProgramme", v)} /><Area label="Méthodes et moyens pédagogiques" value={f.formationMethodes} onChange={(v) => set("formationMethodes", v)} /><Area label="Modalités d’évaluation des acquis" value={f.formationEvaluation} onChange={(v) => set("formationEvaluation", v)} /><Area label="Accessibilité / besoins spécifiques" value={f.formationAccessibilite} onChange={(v) => set("formationAccessibilite", v)} /><div className="rounded-2xl border border-stone-800 bg-stone-950/40 p-4"><p className="mb-4 text-sm font-medium text-stone-200">Indicateurs de résultats</p><div className="grid gap-4 md:grid-cols-3"><Field label="Taux de satisfaction" placeholder="Ex. 96 %" value={f.indicateurSatisfaction} onChange={(v) => set("indicateurSatisfaction", v)} /><Field label="Nombre d’apprenants" placeholder="Ex. 42" value={f.indicateurApprenants} onChange={(v) => set("indicateurApprenants", v)} /><Field label="Taux de réussite" placeholder="Ex. 92 %" value={f.indicateurReussite} onChange={(v) => set("indicateurReussite", v)} /></div><p className="mt-3 text-xs text-stone-500">Si un indicateur n’est pas renseigné, le programme affichera « En cours ».</p></div></div></Card>
      ...
      <section className="rounded-3xl border border-amber-300/20 bg-amber-300/[0.06] p-6"><h2 className="font-semibold text-stone-100">Documents générés</h2><p className="mt-2 text-sm leading-6 text-stone-400">Programme + inscription/positionnement, convention avec règlement intérieur, convocation avec livret d’accueil et règlement intérieur, feuille d’émargement avec attestation de remise des supports, questionnaire de satisfaction, fiche de suivi pédagogique et procédure d’utilisation.</p><p className="mt-2 text-sm leading-6 text-stone-500">Le programme reprend désormais les modalités et délais d’accès, le tarif, les contacts et les indicateurs de résultats. Chaque page conserve le pied de page de l’organisme et sa date d’actualisation.</p></section>
